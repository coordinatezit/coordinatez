...
export function ContactForm() {
  ...
  // Prefill from CTA deep-links: /contact?topic=Scrap Metal Export
  useEffect(() => {
    ...
    try {
      ...
      const res = await fetch(FORMSPREE_ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          "Inquiry Type": "Global Trade",
          Country: data.country,
          Message: data.message,
          "Source Page": document.referrer || "Direct / bookmarked",
          _subject: `New Global Trade inquiry from ${data.name}`,
      ...
      trackEvent("contact_form_submit", { interest: "Global Trade" });
      setStatus("success");
      resetForm();
    } catch {
      setStatus("idle");
  ...
  if (status === "success") {
    return (
      <FormSuccessCard
        title="Thank You for Contacting Coordinatez Global Trade"
      ...
      <FieldGroup>
        ...
        <Field data-invalid={!!errors.message}>
          ...
          <Textarea
            id="contact-message"
            rows={5}
            placeholder="Tell us about your requirement — material, specification or grade, quantity, and destination…"
            {...register("message")}
          />
