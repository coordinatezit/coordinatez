...
const nextConfig: NextConfig = {
  // This app lives inside the www repo (trade/), so lockfile-based workspace-root
  // detection would otherwise pick the parent directory. Pin the root explicitly.
  turbopack: {
    root: __dirname,
  },
  ...
  output: "standalone",
  // Permanent redirects for legacy paths (the trade pages lived under
  // /global-trade/* on www.coordinatez.com) so old inbound links and
  // search-engine memory land on the current equivalent (single hop).
  async redirects() {
    return [
      { source: "/global-trade", destination: "/", permanent: true },
      { source: "/global-trade/:slug", destination: "/:slug", permanent: true },
    ];
  },
  async headers() {
    return [
